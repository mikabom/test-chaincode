...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "create_data" {
		return t.create_data(stub, args[0], args[1], args[2])
	}
...
func (t *SimpleChaincode) create_data(stub shim.ChaincodeStubInterface, userID string, day string, count string) ([]byte, error) {

	if userID == "" {
		fmt.Printf("CREATE_DATA: Invalid userID provided")
		return nil, errors.New("Invalid userID provided")
	}

	var hosuu HosuuData
	hosuu.Day = day
	hosuu.Count, _ = strconv.Atoi(count)
	//day := "\"Day\":\"" + "2017.04.02" + "\", "
	//count := "\"Count\":" + 8240
	...
	if err != nil {
		fmt.Print("Error creating Data_Holder record")
	}
...
func (t *SimpleChaincode) get_data(stub shim.ChaincodeStubInterface, userID string) ([]byte, error) {

	bytes, err := stub.GetState(userID)

	if err != nil {
		return nil, errors.New("Unable to get Data_Holder")
	}

	var data Data_Holder
	...
	if err != nil {
		return nil, errors.New("Corrupt json.marshal data")
	}
